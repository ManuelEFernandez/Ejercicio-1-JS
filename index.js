...
const mostrarImpares = (pizzas) => {
  pizzas.map((pizza) => {
    if (pizza.id % 2 !== 0) {
      console.log(
        `La pizza ${pizza.nombre} tiene como id ${pizza.id}, que es un número impar.`
      );
    }
  });
};
...
const pizza600 = (pizzas) => {
  pizzas.map((pizza) => {
    let precio = 600;
    if (pizza.precio < precio) {
      console.log(
        `Hay pizzas por menos de ${"$" + precio} (la pizza ${
          pizza.nombre
        } vale ${"$" + pizza.precio}).`
      );
    }
...
const juntarNombresPizzas = (pizzas) => {
  pizzas.map((pizza) => {
...
const juntarPreciosPizzas = (pizzas) => {
  pizzas.map((pizza) => {
    numPizzas = Object.keys(pizzas).length;
